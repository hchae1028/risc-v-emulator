...
int main() {
	...
	execute_instruction(addi_cpu, instr, memory);
	...
	execute_instruction(sub_cpu, instr, memory);
	...
	execute_instruction(bitwise_cpu, instr, memory);
	...
	execute_instruction(immediate_bitwise_cpu, instr, memory);
	...
	execute_instruction(shift_cpu, instr, memory);
	...
	execute_instruction(immediate_shift_cpu, instr, memory);
	...
	execute_instruction(comparison_cpu, instr, memory);
	...
	execute_instruction(immediate_comparison_cpu, instr, memory);
	...
	auto branch_target{ execute_instruction(branch_cpu, instr, branch_memory) };
	...
	branch_target = execute_instruction(branch_cpu, instr, branch_memory);
	assert(!branch_target.has_value());
	assert(branch_cpu.read_pc() == 100);
	...
	auto signed_branch_target{ execute_instruction(signed_branch_cpu, instr, signed_branch_memory) };
	...
	signed_branch_target = execute_instruction(signed_branch_cpu, instr, signed_branch_memory);
	assert(!signed_branch_target.has_value());
	...
	auto unsigned_branch_target{ execute_instruction(unsigned_branch_cpu, instr, unsigned_branch_memory) };
	...
	unsigned_branch_target = execute_instruction(unsigned_branch_cpu, instr, unsigned_branch_memory);
	assert(unsigned_branch_target.has_value());
	...
	auto lui_target{ execute_instruction(lui_cpu, instr, lui_memory) };
	...
	lui_target = execute_instruction(lui_cpu, instr, lui_memory);
	...
	auto auipc_target{ execute_instruction(auipc_cpu, instr, auipc_memory) };
	...
	auipc_target = execute_instruction(auipc_cpu, instr, auipc_memory);
	...
	auto jal_target{ execute_instruction(jal_cpu, instr, jal_memory) };
	...
	jal_target = execute_instruction(jal_cpu, instr, jal_memory);
	assert(jal_target.has_value());
	...
	auto jalr_target{ execute_instruction(jalr_cpu, instr, jalr_memory) };
	...
	jalr_target = execute_instruction(jalr_cpu, instr, jalr_memory);
	assert(jalr_target.has_value());
	...
	auto fence_target{ execute_instruction(fence_cpu, instr, fence_memory) };
	assert(!fence_target.has_value());
	instr = decode_instruction(0x0330000F); // FENCE RW, RW
	fence_target = execute_instruction(fence_cpu, instr, fence_memory);
	assert(!fence_target.has_value());
